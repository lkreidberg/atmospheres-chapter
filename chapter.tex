...
The precision of a light curve is often quantified by how close it is to the photon-limited noise floor. Space-based observations often reach the photon limit \citep{sing11, kreidberg14a, ingalls16}, whereas ground-based observations are typically a factor of a few above it \citep[e.g.][]{bean13}.

\runinhead{Instrument Systematics}
The ideal instrument would record every incident photon. In reality, detectors have varying sensitivity 

Modern detectors have come a long way in achieving this goal, but they are not perfect. Here we summarize a few common effects that 

All the usual steps in data reduction must be taken: correcting for bias, dark current, and flatfielding. We focus here on specific noise sources that are not commonly taken into account, as they are only relevant for the special case of sub-percent precision spectrophotometry.

\begin{itemize}
\item{\emph{Charge trapping}.}
\item{\emph{Intrapixel effect.}}
\end{itemize}

\runinhead{Astrophysical Systematics}
There are also 
\begin{itemize}
\item{\emph{Background stars.} Roughly half of stars have one or more bound companions \citep{raghavan10}. If the companion flux is blended with that of the host star, it dilutes the planet signal. Transit and eclipse depths must be multiplied by a correction factor $1 + \beta(\lambda)$ where $\beta$ is the ratio of the background star to host star flux. High contrast imaging is often needed to detect close companions \citep[e.g.][]{bechter14}, and should be obtained for systems that are targets for atmosphere characterization.}
\item{\emph{Stellar activity.} No star is a perfectly constant light source; over a stellar rotation period (typically weeks to months), star spots rotate in and out of view, and total spot coverage changes over longer timescales. The effect on the transit depth.}
\item{\emph{Unocculted star spots.} Even for observations taken at a single epoch, unocculted spots can bias the transmission spectrum. To first order, the difference in temperature between the spot and the stellar photosphere introduces a slope in the planet spectrum. If the spots are cool enough for molecules to form, they can even produce spurious spectral features, as there is no way to distinguish absorption due to the spot from absorption by the atmosphere. The effect from spots can be corrected by multiplying the planet spectrum by a factor $(1−s(1−F_\lambda(\mathrm{spot})/F_\lambda(\mathrm{phot})))^{-1}$, where $s$ is the spot covering fraction and $F_\lambda$ is the spectral radiance \citep{mccullough14}. The spot properties are not usually known, but can be estimated based on the amplitude of photometric variability (FIXME cite Pont, Sing?) } 
 
\item{\emph{Incorrect limb darkening.}}
\item{\emph{Planet .}}
\end{itemize}
