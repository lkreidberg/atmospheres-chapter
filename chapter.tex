...
Stars are both a help and a hindrance to characterizing exoplanet atmospheres.  With the direct imaging approach described in Chapter FIXME, great effort must be undertaken to mask the star, which can outshine the planet by more than a million to one. 

Alternatively, one can analyze the combined light of the planet \emph{and} the star together. For this method, the star serves as a constant baseline flux source. As the planet moves through its orbit, it absorbs, re-emits, and reflects stellar light. These planetary signals can be measured differentially relative to the baseline stellar flux over time.  This strategy has been used to infer the presence , In this chapter, we overview observing techniques, observing facilities, major results.
