...
FIXME: consolidate with previous section
...
\item{\emph{Tidal distortion.}}
\end{itemize}

\section{Major Results from Atmosphere Studies}
In this section we overview the state of the art 

\subsection{Expectations}
Before jumping into observational results, let us set the stage with some a priori predictions for what we expect to see. 

A first question is, what are the basic building blocks of planet formation?  We can make 

primary atmosphere, accreted from the protoplanetary disk, should be rich in hydrogen and helium, which are the dominant component of the disk by mass. These planets could form either by core accretion \citep{pollack96}, or gravitational collapse \citep{}.


We know from the planets of the Solar System that a wide range of chemical compositions are possible. However, we can make some educated guesses about basic atmospheric constituents based on the building blocks of planet formation.

Mass and radius measurements

temperature structure

A third feature of Solar System
...
The bulk composition of gas giants planets is dominated by hydrogen and helium.

After hydrogen and helium, the most abundant elements are carbon and oxygen \citep{anders89}. At the temperatures and pressures typical for planetary atmosphers, these elements are generally expected to be found in molecular form, e.g., water (H$_2$O), methane (CH$_4$), carbon monoxide (CO), and carbon dioxide (CO$_2$).
...
The first of these is the Transiting Exoplanet Survey Satellite (\tess), a planet finding mission scheduled to launch in 2018 \citep{ricker14}.  The goal of \tess\ is to search the 200,000 brightest stars in the sky for transiting planets, with an expected yield of nearly 2000 \citep{sullivan15}. In contrast to the majority of transiting planets discovered to date by \Kepler \citep{}, the \tess\ planets will have bright enough host stars for precise atmosphere characterization. The sample will also include longer period and smaller planets than are typically discovered by ground-based surveys of bright stars, opening the door to studying a more diverse sample.

The second game-changer is \jwst. The primary technical limitations in atmosphere characterization thus far have been aperture size and wavelength coverage, and \jwst\ offers major improvements on both fronts. It has roughly ten times the collecting area of \hst\, and provides spectroscopy from the optical to the infrared ($0.6 - 28\,\mu$m). These improvements will make it possible to study giant planets at higher S/N and push to sub-Neptune to  and smaller planets . The expanded wavelength coverage will enable the first unambiguous detection of many molecules (including methane, carbon dioxide, and ammonia). With \jwst's infrared sensitivity we will detect thermal emission from cooler objects, perhaps including some in the habitable zone.
...
transmission spectroscopy is gonna be hard
nearest transiting, habitable-zone terrestrial planet is expected to be within FIXME parsecs \citep{dressing16}.
