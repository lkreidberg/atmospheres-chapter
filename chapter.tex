...
\subsection{Aerosols}
Aerosols -- an umbrella term for clouds and hazes -- are ubiquitous in the Solar System and are proving to be common in exoplanets as well. Transmission spectra are particularly sensitive to the presence of aerosols, due to the slant viewing geometry observed during transit \citep{fortney05}.  Most aerosols have uniform (so-called "gray") opacity over relatively wide wavelength regions, and mask transmission of stellar flux below a constant altitude.  Therefore one of the main effects on transmission spectra is to truncate absorption features (as illustrated in Figure FIXME). Over wider wavelength intervals ($1-5\,\mu$m), aerosols can produce 
 
In addition, Rayleigh scattering also introduce a steep slope in the optical spectrum \citep{FIXME}.

The first hint of aerosols came from HD 209458b, which had a smaller than expected sodium feature relative to a clear atmosphere \citep{charbonneau02}. This observation foreshadowed a slew of spectra with truncated features \citep[e.g.][]{deming13, kreidberg14a, knutson14a, kreidberg15b}. Many spectra have also shown a large optical slope

There is no strong correlation between the presence of clouds and any other physical parameters (temperature, surface gravity) \citep{sing16}. 
...
Additional evidence for aerosols comes from optical wavelength transmission spectra.

Most of \HST/STIS
ground-based observations \citep{dragomir16}

Reflective clouds have also been inferred from optical phase curves of hot Jupiters. Some planets show westward shifted peak brightnesses, which is best  
