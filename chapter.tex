...
FIXME: say what a transit light curve is and show a figure
...
One approach is to use comparison stars to correct for systematic trends in the target light curve. For photometry, this is a straightforward process: in addition to the target, several nearby stars are observed in the same field of view. The path of their flux traverses similar parts of the atmosphere, and thus their light curves exhibit close-to-identical systematic trends as the target. The target light curve is then divided by the sum of the comparison star light curves to remove systematics. 

The same principle applies to spectroscopic observations, but requires a slighty more specialized approach.  Spectrographs usually image the target through a slit prior to dispersing the light, in order to block out contamination from the sky background and any nearby stars. However, the slits are typically narrower than the seeing disk of the star, and some fraction of flux is lost beyond the slit's edge. These slit losses depend on the seeing and vary with time, preventing precise light curve 

 To work around this problem, one can use a multi-object spectrograph with a custom mask with wide slits ($>10''$), a technique first developed by \cite{bean10}. By observing the target and comparison stars through wide slits, 

There are several spectrographs available that have this capability, including Magellan/LDSS3c, Gemini/GMOS, Keck/, FIXME.  This technique enables close to space-based precision from the ground.

\runinhead{High Resolution Spectrographs}
Another creative technique uses high resolution spectroscopy.

\section{Best Practices for Achieving Precise and Accurate Measurements}
The fundamental limitation to how precise a measurement can be is photon noise. Atmosphere characterization from combined light measurements relies on differential measurement ; requires FIXME photons per 

But there are additional sources of noise - both instrumental and astrophysical - that must be carefully corrected by the observer.
